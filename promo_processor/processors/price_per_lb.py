...
class BasicPricePerLbProcessor(PromoProcessor, version=1):
    ...
    patterns = [
        r'\$(?P<price_per_lb>\d+(?:\.\d{2})?)\s+per\s+lbs?.*',
    ...
    def calculate_deal(self, item, match):
        ...
        if weight:
            ...
            item_data["unit_price"] = base_round(total_price)
    ...
    def calculate_coupon(self, item, match):
        ...
        if weight:
            ...
            item_data["digital_coupon_price"] = base_round(coupon_savings)
            item_data["unit_price"] = base_round(coupon_savings)
